...
const InputCard = () => {
  ...
  const selectTip = (value) => {
    setValues({ ...values, custom: "", percentage: value });
  };
          ...
          <div className="grid grid-cols-2 gap-4 text-white text-center text-2xl font-semibold md:grid-cols-3 ">
            {percentages.map((percentage, index) => (
              <button
                ...
                className={
                  values.custom
                    ? "bg-[#0c494c] py-2 rounded-lg"
                    : values.percentage == percentage
                    ? "bg-[#30c1ad] text-[#0c494c] py-2 rounded-lg"
                    : "bg-[#0c494c] py-2 rounded-lg"
                }
              >
                {percentage}%
            ...
            <input
              ...
              name="custom"
              value={values.custom || ""}
              className="text-xl rounded-lg text-[#0c494c] bg-[#f2f8fb] focus:outline-[#30c1ad] text-center hover:cursor-pointer "
              placeholder="Custom"
            />
          </div>
